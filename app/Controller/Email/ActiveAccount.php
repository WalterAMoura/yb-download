...
class ActiveAccount extends Page
{
    /**
    ...
    public static function getActiveAccount(Request $request): string
    {
        ...
        if($tokenDecrypt['status'] === 'unauthorized' || $tokenDecrypt['decrypt']['customScopes']->path !== $path){
            ...
            if(self::setActiveUser($tokenDecrypt['decrypt']['id'])){
                ...
                $obPasswordTemp = EntityPasswordTemp::getPasswordTempByUserId($tokenDecrypt['decrypt']['id']);
                $obUser = EntityUser::getUserById($tokenDecrypt['decrypt']['id']);

                self::sendEmailSuccessActiveAccount($obUser->email,$obUser->name, $tokenDecrypt['decrypt']['iss'],$obUser->login,PasswordEncryptor::decryptPassword($obPasswordTemp->password));
