...
const random = '{{uuid}}';
const id_II = '{{id}}';
const barChartCanvasI = $(id_II).get(0).getContext('2d')
const barChartDataI =  {
...
const barChartOptionsI = {
    responsive              : true,
    maintainAspectRatio     : false,
    datasetFill             : false,
    onClick: function (evt, item) {
        if (item.length) {
            let dataset = barChartDataI.datasets[item[0]._datasetIndex];
...
const barChartI = new Chart(barChartCanvasI, {
    type: 'bar',
    data: barChartDataI,
    options: barChartOptionsI
})